...
set @fechaCorte = date_sub( concat(year(curdate()),'-',month(curdate()),'-01'), interval if(day(curdate())<=10,1,0) month );
...
where FuncionAsignadaCompEstado = 'P'
  ...
  -- considero los registros de hasta el mes pasado, y los primeros 10 días del mes considero hasta hace dos meses
  and (FuncionAsignadaCompAnio < year(@fechaCorte)
       or FuncionAsignadaCompAnio = year(@fechaCorte)
          and FuncionAsignadaCompMes < month(@fechaCorte)
      )
  and TipoCompId = 1;
