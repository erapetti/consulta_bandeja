...
def main(inicio,fin,correcciones,ci):
    ...
    query_cesesxretoma_novedad = sa.select([relaciones_laborales.c.PersonalPerId,suplencias.c.RelLabId,relaciones_laborales.c.RelLabId,relaciones_laborales.c.RelLabCeseFchReal,relaciones_laborales.c.RelLabFchIniActividades]).select_from(suplencias.join(relaciones_laborales, suplencias.c.SuplRelLabId==relaciones_laborales.c.RelLabId)).where(((suplencias.c.SuplCausId==6) | (suplencias.c.SuplCausId==39))&(relaciones_laborales.c.RelLabCeseFchReal >= sa.bindparam('p1d')) & (relaciones_laborales.c.RelLabCeseFchReal < sa.bindparam('p2d'))&(relaciones_laborales.c.CauBajCod=='66')&(relaciones_laborales.c.RelLabAnulada==0))
    query_cesesxretoma_cr = sa.select([relaciones_laborales.c.PersonalPerId,suplencias.c.RelLabId,relaciones_laborales.c.RelLabId,relaciones_laborales.c.RelLabCeseFchReal,relaciones_laborales.c.RelLabFchIniActividades]).select_from(suplencias.join(relaciones_laborales, suplencias.c.SuplRelLabId==relaciones_laborales.c.RelLabId)).where(((suplencias.c.SuplCausId==6) | (suplencias.c.SuplCausId==39))&(relaciones_laborales.c.RelLabCeseFchReal < sa.bindparam('p1d')) & ((cast(relaciones_laborales.c.RelLabCeseFchAlta,Date) >= sa.bindparam('p1d'))&(cast(relaciones_laborales.c.RelLabCeseFchAlta,Date) < sa.bindparam('p2d')))&(relaciones_laborales.c.CauBajCod=='66')&(relaciones_laborales.c.RelLabAnulada==0))
    ...
    if len(df_cesesxretoma['RelLabIdT'].unique().tolist()) > 0: #si hay algun cese por retoma (evito warning performance)
        ...
        for ix, row in df_cesesxretoma.iterrows():
            if row['RelLabFchIniActividadesS'] and row['RelLabFchIniActividadesS']<row['RelLabFchCeseRealS']:
    ...
    def cambiarAsignatura151porPuesto(row):
        if (row['HorClaAsiCod']=='151') and (not np.isnan(row['PuestoAsignId'])):
