...
def main(inicio,fin,correcciones,ci):
    ...
    parametros['inicioLectivo']=dt.datetime(2019,03,01)
    ...
    suspensiones = sa.Table('suspensiones',metadata, autoload=True,autoload_with=engine_bandeja_in)
    ...
    df_suspensiones = pd.read_sql_query(query_tabla_suspensiones, engine_bandeja_in,params=parametros)
    ...
    query_suplencias = sa.select([relaciones_laborales.c.PersonalPerId,suplencias.c.RelLabId,suplencias.c.SuplFchAlta]).select_from(suplencias.join(relaciones_laborales.join(puestos, relaciones_laborales.c.PuestoId == puestos.c.PuestoId), suplencias.c.RelLabId==relaciones_laborales.c.RelLabId)).where((suplencias.c.SuplFchAlta >= sa.bindparam('pInis')) & (suplencias.c.SuplFchAlta < sa.bindparam('p2d')) & (puestos.c.PuestoFuncionId.in_(puestos_funcion)) & ((suplencias.c.SuplCausId==6) | (suplencias.c.SuplCausId==39) | (suplencias.c.SuplCausId==43)))# suplenciaId = 6 reserva de cargo, = 10 licencia sin sueldo
    ...
    query_cesesxretoma_novedad = sa.select([relaciones_laborales.c.PersonalPerId,suplencias.c.RelLabId,relaciones_laborales.c.RelLabId,relaciones_laborales.c.RelLabCeseFchReal,relaciones_laborales.c.RelLabFchIniActividades]).select_from(suplencias.join(relaciones_laborales, suplencias.c.SuplRelLabId==relaciones_laborales.c.RelLabId)).where(((suplencias.c.SuplCausId==6) | (suplencias.c.SuplCausId==39) | (suplencias.c.SuplCausId==43))&(relaciones_laborales.c.RelLabCeseFchReal >= sa.bindparam('p1d')) & (relaciones_laborales.c.RelLabCeseFchReal < sa.bindparam('p2d'))&(relaciones_laborales.c.CauBajCod=='66')&(relaciones_laborales.c.RelLabAnulada==0))
    query_cesesxretoma_cr = sa.select([relaciones_laborales.c.PersonalPerId,suplencias.c.RelLabId,relaciones_laborales.c.RelLabId,relaciones_laborales.c.RelLabCeseFchReal,relaciones_laborales.c.RelLabFchIniActividades]).select_from(suplencias.join(relaciones_laborales, suplencias.c.SuplRelLabId==relaciones_laborales.c.RelLabId)).where(((suplencias.c.SuplCausId==6) | (suplencias.c.SuplCausId==39) | (suplencias.c.SuplCausId==43))&(relaciones_laborales.c.RelLabCeseFchReal < sa.bindparam('p1d')) & ((cast(relaciones_laborales.c.RelLabCeseFchAlta,Date) >= sa.bindparam('p1d'))&(cast(relaciones_laborales.c.RelLabCeseFchAlta,Date) < sa.bindparam('p2d')))&(relaciones_laborales.c.CauBajCod=='66')&(relaciones_laborales.c.RelLabAnulada==0))
    ...
    df_cesesxretoma_novedad.columns = ['peridS','RelLabIdT','RelLabIdS','RelLabFchCeseRealS','RelLabFchIniActividadesS']
    df_cesesxretoma_cr.columns = ['peridS','RelLabIdT','RelLabIdS','RelLabFchCeseRealS','RelLabFchIniActividadesS']
    ...
    if len(df_cesesxretoma['RelLabIdT'].unique().tolist()) > 0: #si hay algun cese por retoma (evito warning performance)
        ...
        for ix, row in df_cesesxretoma.iterrows():
            if row['RelLabFchIniActividadesS'] and row['RelLabFchCeseRealS'] and row['RelLabFchIniActividadesS']<row['RelLabFchCeseRealS']:
                df_altasxretoma_titulares.loc[(df_altasxretoma_titulares['RelLabId']==row['RelLabIdT']) & (df_altasxretoma_titulares['falta']<row['RelLabFchCeseRealS']),['falta']]=row['RelLabFchCeseRealS']+dt.timedelta(days=1)
    ...
    for ix, row in df_suplencias.iterrows():
        df_historia_rl.loc[df_historia_rl['RelLabId']==row['RelLabId'],['fcese']]=row['SuplFchAlta'].date()-dt.timedelta(days=1)
    ...
    df_historia_completa = df_historia_completa.loc[:,['ci','nombre','dependid','asignid','ciclo','caracter','horas','turno','falta','fcese','causal','grupo','FuncionId','anulada','perid','PuestoAsignId','RelLabId']]
    # atributos harcoded
    ...
    # df_historia_completa.loc[(df_historia_completa['HorClaAsiCod']=='75')&(df_historia_completa['HorClaFchCese'].isnull()),['HorClaFchCese','HorClaFchLib']]=date(cfg['cese_hc_anio'],cfg['cese_hc_mes'],cfg['cese_hc_dia'])
    #Agrego la materia 162 para las Juntas Médicas
    df_juntamedica = df_suspensiones[df_suspensiones['motivo']=='Junta M\xe9dica']
    df_historia_completa.loc[df_historia_completa['PerDocNum'].isin(df_juntamedica['PerDocNum']),'HorClaAsiCod']='162'

    #Cambio la dependencia para los pases en comisión
    df_pasesfuera = df_suspensiones[df_suspensiones['motivo']=='SIAP COMISION FUERA SECUN']
    df_historia_completa.loc[df_historia_completa['PerDocNum'].isin(df_pasesfuera['PerDocNum']),'HorClaInsCod']='25.3.0.0.8901'
    df_pasesdentro = df_suspensiones[df_suspensiones['motivo']=='SIAP COMISION DENTRO ANEP']
    df_historia_completa.loc[df_historia_completa['PerDocNum'].isin(df_pasesdentro['PerDocNum']),'HorClaInsCod']='25.3.0.0.8902'
    ...
    def cambiarAsignatura151porPuesto(row):
        if (row['HorClaAsiCod']=='151') and row['PuestoAsignId']<>None and (not np.isnan(row['PuestoAsignId'])):
            return str(row['PuestoAsignId']).split('.')[0]
        else:
            return row['HorClaAsiCod']
    
    df_historia_completa['HorClaAsiCod']=df_historia_completa.apply(cambiarAsignatura151porPuesto, axis=1)
    del df_historia_completa['PuestoAsignId']
