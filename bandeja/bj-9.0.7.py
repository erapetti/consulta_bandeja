...
def main(inicio,fin,ci):
    ...
    query_novedades = sa. \
        ...
        where( \
              ...
              (relaciones_laborales.c.RelLabFchIniActividades >= sa.bindparam('inicioLectivo')) & \
              (relaciones_laborales.c.RelLabFchIniActividades < sa.bindparam('tope')) \
             )
    ...
    j3 = rlt.join(puestos).join(funciones_relacion_laboral).join(funciones_asignadas, ((rlt.c.SillaId==funciones_asignadas.c.SillaId) & (funciones_relacion_laboral.c.FuncAsignadaId==funciones_asignadas.c.FuncAsignadaId))).join(sillas).join(silla_grupo_materia, sillas.c.SillaId==silla_grupo_materia.c.SillaId, isouter=True).join(asignaturas_materias, sillas.c.MateriaId==asignaturas_materias.c.MateriaId, isouter=True)
    ...
    query_alta_luego_de_suplencia = sa. \
        ...
        select_from( \
            ...
            join(jsupl_intermedia, \
                 ((supl_intermedia.c.RelLabId==rlt.c.RelLabId) & (supl_intermedia.c.SuplId<>suplencias.c.SuplId) & (supl_intermedia.c.SuplFchAlta>=suplencias.c.SuplFchAlta) & (supl_intermedia.c.SuplId<>supl_siguiente.c.SuplId) & ((supl_intermedia.c.SuplFchAlta<supl_siguiente.c.SuplFchAlta) | ((supl_intermedia.c.SuplFchAlta==supl_siguiente.c.SuplFchAlta) & (supl_intermedia.c.SuplId<supl_siguiente.c.SuplId)))), \
                 isouter=True) \
        ). \
        where( \
    ...
    query_primera_suplencia = sa. \
        ...
        select_from(
            ...
            join(j3, ((rlt.c.RelLabId==suplencias.c.RelLabId) & (rlt.c.RelLabAnulada==0))). \
            join(jsupl_intermedia, \
                 ((supl_intermedia.c.RelLabId==rlt.c.RelLabId) & (supl_intermedia.c.SuplId<>suplencias.c.SuplId) & ((supl_intermedia.c.SuplFchAlta<suplencias.c.SuplFchAlta) | ((supl_intermedia.c.SuplFchAlta==suplencias.c.SuplFchAlta) & (supl_intermedia.c.SuplId<suplencias.c.SuplId)))),
                 isouter=True) \
        ). \
        where( \
            (rlt.c.RelLabFchIniActividades >= sa.bindparam('inicioLectivo')) & \
