...
def main(inicio,fin,correcciones,ci):
    ...
    engine = create_engine('mysql+pymysql://'+cfg['personal']['usr']+':'+cfg['personal']['password']+'@'+cfg['personal']['host']+'/'+cfg['personal']['bd'])
    engine_personas = create_engine('mysql+pymysql://'+cfg['personas']['usr']+':'+cfg['personas']['password']+'@'+cfg['personas']['host']+'/'+cfg['personas']['bd'])
    engine_estudiantil = create_engine('mysql+pymysql://'+cfg['estudiantil']['usr']+':'+cfg['estudiantil']['password']+'@'+cfg['estudiantil']['host']+'/'+cfg['estudiantil']['bd'])
    engine_bandeja = create_engine('mysql+pymysql://'+cfg['bandeja']['usr']+':'+cfg['bandeja']['password']+'@'+cfg['bandeja']['host']+'/'+cfg['bandeja']['bd'])

    p1 = dt.datetime(int(inicio.split('-')[0]),int(inicio.split('-')[1]),int(inicio.split('-')[2]))
    p2 = dt.datetime(int(fin.split('-')[0]),int(fin.split('-')[1]),int(fin.split('-')[2]))

    # los puestos considerados docencia directa
    puestos_funcion = [100, 12397, 12362, 12363, 12364, 12365, 12369, 12370, 12373, 12376, 12377, 12378, 12381, 12382, 12383, 12384, 12398, 100003, 12367, 12375, 12371, 12380, 12379]
    parametros = {}
    parametros['p1d']=p1
    parametros['p2d']=p2
    ...
    parametros['pInis']='2017-09-01'
    #las columnas que seran utilizadas en la salida (al final se cambian por las de SIAP)
    columnas = ['ci','dependid','asignid','caracter','ciclo','horas','falta','fcese','causal','nombre','turno','grupo']
    ...
    #query_cesesxretoma = sa.select([relaciones_laborales.c.PersonalPerId,suplencias.c.RelLabId,relaciones_laborales.c.RelLabId,relaciones_laborales.c.RelLabCeseFchReal]).select_from(suplencias.join(relaciones_laborales.join(puestos, relaciones_laborales.c.PuestoId == puestos.c.PuestoId), suplencias.c.SuplRelLabId==relaciones_laborales.c.RelLabId)).where((puestos.c.PuestoFuncionId.in_(puestos_funcion))&(suplencias.c.SuplCausId==6)&(relaciones_laborales.c.RelLabCeseFchReal >= sa.bindparam('p1d')) & (relaciones_laborales.c.RelLabCeseFchReal < sa.bindparam('p2d')) )
    #df_cesesxretoma= pd.read_sql_query(query_cesesxretoma, engine, params=parametros)
    #df_cesesxretoma.columns = ['peridS','RelLabIdT','RelLabIdS','RelLabFchCeseRealS']

    #query_cesesxretoma_titulares = sa.select([relaciones_laborales.c.PersonalPerId, relaciones_laborales.c.RelLabId]).select_from(relaciones_laborales.join(puestos, relaciones_laborales.c.PuestoId==puestos.c.PuestoId)).where(relaciones_laborales.c.RelLabId.in_(df_cesesxretoma['RelLabIdT'].unique().tolist()))
    #df_cesesxretoma_titulares= pd.read_sql_query(query_cesesxretoma_titulares, engine, params=parametros)
    #
    ...
    for ix, row in df_suplencias.iterrows():
        df_historia_completa.loc[df_historia_completa['RelLabId']==row['RelLabId'],['fcese']]=row['SuplFchAlta'].date()
    ...
    # atributos harcoded
    df_historia_completa.loc[df_historia_completa['FuncionId'].isin(df_funciones_hap.tolist()),'caracter']='I'
    df_historia_completa.loc[df_historia_completa['FuncionId'].isin(df_funciones_hap.tolist()),'asignid']='80'
    df_historia_completa.loc[df_historia_completa['FuncionId'].isin(df_funciones_POB.tolist()),'caracter']='I'
    df_historia_completa.loc[df_historia_completa['FuncionId'].isin(df_funciones_POB.tolist()),'asignid']='150'
    df_historia_completa.loc[df_historia_completa['FuncionId'].isin(df_funciones_POP.tolist()),'caracter']='I'
    df_historia_completa.loc[df_historia_completa['FuncionId'].isin(df_funciones_POP.tolist()),'asignid']='57'
    df_historia_completa.loc[df_historia_completa['FuncionId'].isin(df_funciones_68.tolist()),'caracter']='I'
    df_historia_completa.loc[df_historia_completa['FuncionId'].isin(df_funciones_68.tolist()),'asignid']='68'
    df_historia_completa.loc[df_historia_completa['FuncionId'].isin(df_funciones_talleristas.tolist()),'caracter']='I'
    df_historia_completa.loc[df_historia_completa['FuncionId'].isin(df_funciones_talleristas.tolist()),'asignid']='45'
    df_historia_completa.loc[df_historia_completa['FuncionId'].isin(df_coordinadores_especiales.tolist()),'caracter']='I'
    df_historia_completa.loc[df_historia_completa['FuncionId'].isin(df_coordinadores_especiales.tolist()),'asignid']='75'
    df_historia_completa.loc[(df_historia_completa['asignid']=='75') & (df_historia_completa['fcese'].notnull()),'causal']='50'
    df_historia_completa.loc[(df_historia_completa['anulada']==1),'causal']='99'
    ...
    df_historia_completa.loc[df_historia_completa['asignid'].isnull(),'asignid']='151'
    ...
    df_historia_completa.loc[(df_historia_completa['HorClaAsiCod']=='75')&(df_historia_completa['HorClaFchCese'].isnull()),['HorClaFchCese']]=date(2019,02,28)
    ...
    #df_historia_completa.to_csv('correcciones.csv')
