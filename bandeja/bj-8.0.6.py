...
def main(inicio,fin,correcciones,ci):
    ...
    query_cesesxretoma_cr = sa.select([relaciones_laborales.c.PersonalPerId,suplencias.c.RelLabId,relaciones_laborales.c.RelLabId,relaciones_laborales.c.RelLabCeseFchReal]).select_from(suplencias.join(relaciones_laborales, suplencias.c.SuplRelLabId==relaciones_laborales.c.RelLabId)).where(((suplencias.c.SuplCausId==6) | (suplencias.c.SuplCausId==39))&(relaciones_laborales.c.RelLabCeseFchReal < sa.bindparam('p1d')) & (relaciones_laborales.c.RelLabCeseFchAlta >= sa.bindparam('p1d')&(relaciones_laborales.c.RelLabCeseFchAlta < sa.bindparam('p2d')))&(relaciones_laborales.c.CauBajCod=='66')&(relaciones_laborales.c.RelLabAnulada==0))
    ...
    df_historia_completa = df_historia_completa.loc[:,['ci','nombre','dependid','asignid','ciclo','caracter','horas','turno','falta','fcese','causal','grupo','FuncionId','anulada','perid','PuestoAsignId']]
    # atributos harcoded
